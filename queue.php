...
<html>
<head>
<title>Music Request System LITE - Enqueue Request</title>
...
<?php
	...
	if($admin !== true || $editable !== true)
	{
		...
		die("You are not authorized to queue this post. You should be automatically redirected. If you are not, please <a href=\"index.php\">click here</a>.");
	}
	...
	if(!empty($_GET['s']) && $disabled === false)
	{
		...
		$request=$requests["New"][$req_id];
		$request["Status"]=1;
		$request["LastAction"]=array("QUEUED",time());
		unset($requests["New"][$req_id]);
		$requests["Queued"][$req_id]=$request;
		...
		echo("<p>Request has been queued. Please <a href=\"index.php\">click here</a>.</p>");
		$disabled=true;
	}
?>
<h1>Music Request System LITE</h1>
<h2>Enqueue Request</h2>
<form method="get" action="queue.php">
<input type="hidden" name="s" value="y">
<input type="hidden" name="req" value="<?php echo $request_id; ?>">
<input type="submit" value="Confirm queue" <?php if($disabled === true) { echo "disabled=\"disabled\""; } ?>>
</form>
